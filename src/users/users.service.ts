...
export class UsersService {
  ...
  public async changePassword(user_uuid: string, body: ChangePasswordDTO): Promise<boolean> {
    ...
    if (!this.bcryptProvider.compareSync(body.old_password.trim(), user.password.trim()))
      throw new BadRequestException(ApiError.INVALID_OLD_PASSWORD);

    user.password = this.bcryptProvider.hashSync(body.new_password.trim(), 10);

    const params: Record<string, unknown> = {
      USER_FIRSTNAME: user.first_name
    };

    await this.nodemailerProvider.sendMail(user.email, MailTextSubject.PASSWORD_CHANGED, params);
