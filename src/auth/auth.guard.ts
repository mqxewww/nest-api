...
import { JwtPayload } from "../common/providers/custom-jwt.provider";
...
export class AuthGuard implements CanActivate {
  ...
  public async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const payload = await this.accessJwtProvider.verifyAsync<JwtPayload>(token);
      ...
      switch (true) {
        case error instanceof TokenExpiredError:
          ...
          request["payload"] = this.accessJwtProvider.decode(token);
